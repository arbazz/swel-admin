...
import addAlbum from '../firebase/addAlbum';
...
export default function Album() {
    ...
    const fetchData = async () => {
        const res = await getAllDoc("Album");
        setData(res);
    }
    ...
    const handleAdd = async () => {
        if (name && image) {
            ...
            try {
                ...
                const res = await addAlbum(name, fileLink);
                if (res === true) {
                    alert("sucess");
                    setLoading(false)
                } else {
    ...
    const handleDelete = async (e) => {
        const res = await deleteData("Album", e.docId);
        if (res === "true") {
            alert("Deleted!");
    ...
    return (
        <div>
            ...
            {!add ? <button onClick={() => setAdd(true)}
                className="btn waves-effect waves-light" type="submit" name="action">Add new Album
